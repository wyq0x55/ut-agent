"""离线收集完整 WinAMS 样本并归纳候选规则。

该模块是规则学习工作流的入口，不参与正常 ``gen``/``project`` 生成。
它只读取用户明确提供的 winAMS TestCsv 与对应 Soft 源码，输出每个样本的
解析状态、去掉符号名后的语义模式聚类和保持 ``candidate`` 的规则包。
"""
...
from ut_agent.rules.infer import infer_rule_pack
...
def _derived_profile_rules(records: list[dict[str, Any]], profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if any(item.get("mcdc_combinations") for item in evidence):
        connectors = sorted({
            ...
            for match in [re.search(r"(\&\&|\|\|)", label)]
            if match
        })
        rules.append({
    ...
    stub_columns = sorted({
        column
        for item in records if item.get("status") == "INFERRED"
        for column in item.get("input_columns", [])
        if re.search(r"(?:CALLCNT_|ARG\d+_|AMIN_return)", column)
    })
    if stub_columns:
        rules.append({
            "id": f"profile.{profile['profile_version']}.stub-contract",
            "status": "candidate",
            "scope": {"function": "*"},
            "match": {"kind": "profile_strategy", "strategy": "stub-contract"},
            "action": {
                "strategy": "preserve-call-count-return-argument-columns",
                "column_examples": stub_columns[:50],
            },
            "priority": 30,
            "evidence": ["history:TestCsv #COMMENT", "基准书:PSD再構築/3-1~3-3"],
            "approval": {},
        })
