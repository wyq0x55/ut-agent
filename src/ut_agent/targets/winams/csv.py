...
from ut_agent.generation.model import GenerationResult, TestIntent
...
def _pointer_column_key(
    ...
    while the generation layer uses the canonical ``pointee:value`` key for
    the same AST access path (``*name``). Keep that target spelling at the
...
def render_suite_csv(ir: FunctionIR, suite, *,
    ...
    rendered = render_intents_csv(
        ir, result, source_label=source_label, title=title,
    )
    from .validation import validate_csv_text
    checked = validate_csv_text(rendered)
    if not checked.valid:
        raise ValueError("WinAMS CSV 格式校验失败: " + "; ".join(checked.errors))
