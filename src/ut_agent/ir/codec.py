...
def _function_dict(ir: FunctionIR) -> tuple[dict[str, Any], bool]:
    ...
    for branch in raw["branches"]:
        ...
        branches.append({
            ...
            "parent_bid": branch["parent_bid"],
            "parent_outcome": branch.get("parent_outcome"),
...
def _read_global_object(value: Mapping[str, Any]) -> GlobalObject:
    return GlobalObject(
        ...
        array_sizes=list(value["array_sizes"]),
        index_drivers=list(value.get("index_drivers", [])),
        field_paths=list(value["field_paths"]),
